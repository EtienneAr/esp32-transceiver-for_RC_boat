...
void motorControl_setSpeed(int speed) {
	...
	if(speed > 0) {
		pulse_width_us =  speed * (CONFIG_MOTORCONTROL_WIDTH_MAX_US - CONFIG_MOTORCONTROL_WIDTH_MID_US) / 1000 + CONFIG_MOTORCONTROL_WIDTH_MID_US;
	} else {
		pulse_width_us = -speed * (CONFIG_MOTORCONTROL_WIDTH_MIN_US - CONFIG_MOTORCONTROL_WIDTH_MID_US) / 1000 + CONFIG_MOTORCONTROL_WIDTH_MID_US;
	}
